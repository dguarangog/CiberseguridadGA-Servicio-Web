...
class MongoDriver:
    ...
    def consulta_record(self,username: str):
        registros = self.client.get_database('pry_prueba_fin').get_collection(f'{username}_PRUEBA-FINAL').find({})
        return registros
    def consulta_record_one(self,record: dict, username: str):
        registros = self.client.get_database('pry_prueba_fin').get_collection(f'{username}_PRUEBA-FINAL').find_one(record)
        return registros
